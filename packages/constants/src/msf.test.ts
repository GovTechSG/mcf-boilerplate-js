...
describe('icms to msf mapping', () => {
  describe('employment types', () => {
    it('should map to Part Time when id is 440002', () => {
      expect(mapIcmsToMsfEmploymentType(440002)).to.deep.equal({ilpId: '05', ilpDescription: 'Part Time'});
    });
    it('should map to Permanent when id is 440003', () => {
      expect(mapIcmsToMsfEmploymentType(440003)).to.deep.equal({ilpId: '07', ilpDescription: 'Permanent'});
    });
    it('should map to Temporary when id is 440004', () => {
      expect(mapIcmsToMsfEmploymentType(440004)).to.deep.equal({ilpId: '02', ilpDescription: 'Temporary'});
    });
    it('should map to Contract Basis when id is 440005', () => {
      expect(mapIcmsToMsfEmploymentType(440005)).to.deep.equal({ilpId: '03', ilpDescription: 'Contract Basis'});
    });
    it('should map to Freelance when id is 440006', () => {
      expect(mapIcmsToMsfEmploymentType(440006)).to.deep.equal({ilpId: '06', ilpDescription: 'Freelance'});
    });
    it('should map to Full Time when id is 440007', () => {
      expect(mapIcmsToMsfEmploymentType(440007)).to.deep.equal({ilpId: '08', ilpDescription: 'Full Time'});
    });
    it('should map to Flexi Time when id is 440008', () => {
      expect(mapIcmsToMsfEmploymentType(440008)).to.deep.equal({ilpId: '09', ilpDescription: 'Flexi Work'});
    });
    it('should map to Internship when id is 440009', () => {
      expect(mapIcmsToMsfEmploymentType(440009)).to.deep.equal({ilpId: '10', ilpDescription: 'Internship'});
    });
  });
  describe('position levels', () => {
    it('should map to Senior Executive when id is 357009', () => {
      expect(mapIcmsToMsfPositionLevel(357009)).to.deep.equal({jobLevelCode: 8, description: 'Senior Executive'});
    });
    it('should map to Junior Executive when id is 357008', () => {
      expect(mapIcmsToMsfPositionLevel(357008)).to.deep.equal({jobLevelCode: 10, description: 'Junior Executive'});
    });
    it('should map to Non-executive when id is 357007', () => {
      expect(mapIcmsToMsfPositionLevel(357007)).to.deep.equal({jobLevelCode: 11, description: 'Non-executive'});
    });
    it('should map to Manager when id is 357003', () => {
      expect(mapIcmsToMsfPositionLevel(357003)).to.deep.equal({jobLevelCode: 3, description: 'Manager'});
    });
    it('should map to Executive when id is 357006', () => {
      expect(mapIcmsToMsfPositionLevel(357006)).to.deep.equal({jobLevelCode: 9, description: 'Executive'});
    });
    it('should map to Fresh/entry level when id is 357004', () => {
      expect(mapIcmsToMsfPositionLevel(357004)).to.deep.equal({jobLevelCode: 12, description: 'Fresh / Entry level'});
    });
    it('should map to Professional when id is 357005', () => {
      expect(mapIcmsToMsfPositionLevel(357005)).to.deep.equal({jobLevelCode: 7, description: 'Professional'});
    });
    it('should map to Middle Management when id is 357002', () => {
      expect(mapIcmsToMsfPositionLevel(357002)).to.deep.equal({jobLevelCode: 2, description: 'Middle Management'});
    });
    it('should map to Senior Management when id is 357001', () => {
      expect(mapIcmsToMsfPositionLevel(357001)).to.deep.equal({jobLevelCode: 1, description: 'Senior Management'});
    });
  });
  describe('position levels', () => {
